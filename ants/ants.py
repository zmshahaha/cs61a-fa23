...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        distance = 0
        while (distance < self.lower_bound) and (not bee_place.is_hive):
            bee_place = bee_place.entrance
            distance += 1
        while (not bee_place.bees) and (not bee_place.is_hive) and (distance <= self.upper_bound):
            bee_place = bee_place.entrance
            distance += 1
        if bee_place.bees and (not bee_place.is_hive) and (distance <= self.upper_bound):
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        # get bees before reduce health
        bees_to_demage = list(self.place.bees)
        Ant.reduce_health(self, amount)
        reflective_damage = amount
        if self.health <= 0:
            reflective_damage += self.damage
        for bees in bees_to_demage:
            bees.reduce_health(reflective_damage)
        # END Problem 5
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while place_behind:
            if place_behind.ant:
                if not place_behind.ant.doubled:
                    place_behind.ant.double()
                if place_behind.ant.is_container and place_behind.ant.ant_contained and not place_behind.ant.ant_contained.doubled:
